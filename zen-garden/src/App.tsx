  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log("About to parse AI report...");
        console.log("Raw analysis length:", result.raw_analysis.length);
        console.log("Raw analysis preview:", result.raw_analysis.substring(0, 500));
        const parsed = parseAIReport(result.raw_analysis);
        console.log("Parsed report result:", {
          amendments: parsed.amendments.length,
          verifications: parsed.verifications.length,
          disclosures: parsed.disclosures.length,
          personnel: parsed.personnel.length,
          firstAmendment: parsed.amendments[0]
        });
